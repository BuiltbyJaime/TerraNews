document.addEventListener("DOMContentLoaded", () => {
  ...
  // declaring values for my functions to manipulate the search section
  ...
  searchForm.addEventListener("submit", async (e) => {
    ...
    // used AI to help me create the logic to formulate the URL.
    ...
    let url = `/api/news?endpoint=${endpoint}&_=${Date.now()}`;

    if (endpoint === "everything" && keywordInput.value.trim()) {
      url += `&q=${encodeURIComponent(keywordInput.value.trim())}`;
    }
  ...
  countryMarkers.forEach((country) => {
    L.marker(country.coords)
      ...
      .on("click", () => {
        console.log("Marker clicked for:", country.name);
